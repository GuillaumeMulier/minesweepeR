...
# Select a field (highlights the selected field)
selected_field <- function(graphe, abscisse, ordonnee) {
  
  graphe_updated <- graphe +
    annotate("rect", xmin = abscisse - .5, xmax = abscisse + .5, ymin = ordonnee - .5, ymax = ordonnee + .5, 
             color = "#2A9421", fill = "#2A9421", size = 2, alpha = .3)
  return(graphe_updated)
